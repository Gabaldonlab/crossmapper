...
def getSequencesPerOrganism(inFastaFile):
    ...
    """
    with open(inFastaFile, "r") as inFile:
        seqsDir = [
...
def getSequencesPerOrganisms(listFiles):
    ...
    """
    seqs = [getSequencesPerOrganism(oneFile) for oneFile in listFiles]
    return seqs
...
def parseGTFrecord(record, selectedFeatures=None):
    ...
    if selectedFeatures != None:
        return dict(
            (k, recordDict[k] if k in recordDict else None)
    ...
    for oneGTFFile in inGTFFiles:
        ...
        for line in inFile:
            ...
            if recordDict["transcript_id"] is None:
                continue
            if seqsIndex != None:
                transcriptMap[recordDict["transcript_id"]] = seqsIndex[
                    recordDict["seqName"]
                ]
            else:
                transcriptMap[recordDict["transcript_id"]] = recordDict[
                    "seqName"
                ]
        inFile.close()
...
class Counter:
    ...
    def getPerSpCrossMapped(
        self, source, target, countType="Total", percent=False, relative=False
    ):
        if source == target:
            return 0
    ...
    def getSpeciesCorssMapped(self, spName, percent=False):
        ...
        for o_sp, o_spId in self.speciesIds.items():
            if o_sp != spName:
                ...
                if percent:
                    resCount = round(
                        100 * resCount / self.getTotalMappedReads(), 3
                    )
    ...
    def getTotalReads(self):
        if self.totalReadsCount == -1:
            self.totalReadsCount = (
                self.getTotalUniqueMapped()
                + self.getTotalMultiMapped()
                + self.getTotalUnmapped()
            )
        return self.totalReadsCount

    def getTotalMappedReads(self):
        if self.totalMappedReadsCount == -1:
            self.totalMappedReadsCount = (
                self.getTotalUniqueMapped() + self.getTotalMultiMapped()
            )
    ...
    def getReadLenSeries(self, countType="Total", percent=False):
        if countType == "Total":
            ...
            for iRow in range(0, len(self.crossSpTotal)):
                total += (
                    sum(self.crossSpTotal[iRow])
                    - self.crossSpTotal[iRow][iRow]
                )
        elif countType == "Unique":
            total = 0
            for iRow in range(0, len(self.crossSpUnique)):
                total += (
                    sum(self.crossSpUnique[iRow])
                    - self.crossSpUnique[iRow][iRow]
                )
        elif countType == "Multi":
            total = 0
            for iRow in range(0, len(self.crossSpMulti)):
                total += (
                    sum(self.crossSpMulti[iRow])
                    - self.crossSpMulti[iRow][iRow]
                )
        if percent:
    ...
    def summary(self, outFile=sys.stdout):
        ...
        print(
            "\t{0}:\t{1}".format(
                "Totol Reads".ljust(lpad), sum(self.total) + sum(self.unmapped)
            ),
            file=outFile,
        )
        ...
        print(
            "\t{0}:\t{1}".format(
                "Unique Mapped Reads".ljust(lpad), totalUniqueMapped
            ),
            file=outFile,
        )
        ...
        print(
            "\t{0}:\t{1}".format(
                "Multi Mapped Reads".ljust(lpad), totalMultiMapped
            ),
            file=outFile,
        )
        print(
            "\t{0}:\t{1}".format(
                "UnMapped Mapped Reads".ljust(lpad), sum(self.unmapped)
            ),
            file=outFile,
        )
        ...
        for sourceSp in self.speciesIds:
            ...
            for targetSp in self.speciesIds:
                ...
                print(
                    self.crossSpUnique[sourceSpID][targetSpID],
                    end="\t",
                    file=outFile,
                )
            print("\n", file=outFile)
        ...
        for sourceSp in self.speciesIds:
            ...
            for targetSp in self.speciesIds:
                targetSpID = self.speciesIds[targetSp]
                print(
                    self.crossSpMulti[sourceSpID][targetSpID],
                    end="\t",
                    file=outFile,
                )
            print("\n", file=outFile)
        ...
        for sourceSp in self.speciesIds:
            ...
            for targetSp in self.speciesIds:
                targetSpID = self.speciesIds[targetSp]
                print(
                    self.crossSpTotal[sourceSpID][targetSpID],
                    end="\t",
                    file=outFile,
                )
            print("\n", file=outFile)
        ...
        if record.is_secondary:
            if record.is_proper_pair:
                if not record.qname in allReads:
                    allReads[
                        record.qname
                    ] = 1  ## start from one as we ignore the primary mapped read
...
def reportToFile(
    ...
    if referenceMapName.startswith(idToSpName[mappingSpId] + "_"):
        referenceMapName = referenceMapName[
            len(idToSpName[mappingSpId] + "_") :
        ]

    print(
        f"{orgSeqName}\t{read.start}\t{read.end}\t{idToSpName[mappingSpId]}\t{referenceMapName}\t{record.reference_start}\t{record.reference_start}\t{record.cigarstring}\t{rlen}\t{layout}",
        file=reportFile,
    )
    return
    ...
    for record in bamFile.fetch(until_eof=True):
        ...
        if record.reference_name == read.orgSeqName:
            ...
            if orgReadSpId == mappingSpId:
                ...
                counterIndex = (
                    1  ## map to the it org sp but map to the wrong contig
                )
                # print("Yes")
            else:
        ...
        if nhTag != None:
            ...
            if (
                qnameTolookAt in nhTag
            ):  ## if not in nhTag then it is a unique map
...
def getReadCounters(args):
    ...
    allSeqs = getSequencesPerOrganisms(
        spInputFastaFiles
    )  ## testing was [sp1InputFasta,sp2InputFasta]
    ...
    if args.simulation_type == "RNA":
        ...
        transcriptMap = mapTranscriptToSequence(
            [args.annotationsGTFConcat], seqsIndex
        )  ## test was [sp1InputGTF, sp2InputGTF]
